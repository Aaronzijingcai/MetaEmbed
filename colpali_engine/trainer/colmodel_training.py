...
class ColModelTrainingConfig:
    ...
    def __post_init__(self):
        ...
        # update do_padding
        self.do_padding = (
            self.doc_pooling_type is not None
            and self.doc_pooling_type.startswith("split")
        )
...
class ColModelTraining:
    ...
    def train(self) -> None:
        ...
        if self.config.tr_args.resume_from_checkpoint:
            ...
            checkpoint_path = (
                get_last_checkpoint(checkpoint_dir)
    ...
    def eval(self) -> None:
        ...
        if self.config.eval_dataset_loader_v2 is not None:
            for (
                ...
                if any(
                    keyword in test_name for keyword in self.config.no_eval_keywords
                ):
                    continue
