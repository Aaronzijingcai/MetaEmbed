...
# From https://github.com/haon-chen/MoCa/blob/main/src/vlm_backbone/qwen2_5_vl_embed/utils_new.py
# Purpose of this util is to truncate the input_ids to the maximum length
# and manipulate the image_grid_thw to reflect the change on image side
class Truncation:
    def __init__(
        self,
        content: int = 151655,
        ...
        train: bool = False,  # if True, will add an empty image to current batch if no images present
    ):
    ...
    def sanity_check(self, input_ids, attention_mask, pixel_values, image_grid_thw):
    ...
    def truncate(self, inputs: dict, length: int = 32768) -> dict:
...
class SafeTruncation(Truncation):
    """
    A safe truncation class that only adds black images and does not truncate anything
    As we ensure the maximum of visual tokens, truncation can be safely done in the processor itself
    with `processor_max_length`
    """
    ...
    def truncate(self, inputs: dict, length: int = 32768) -> dict:
        ...
        if pixel_values is None and image_grid_thw is None:
            ...
            # print("do nothing as we have images...")
            return {
                "input_ids": input_ids,
                "attention_mask": attention_mask,
                "pixel_values": pixel_values,
