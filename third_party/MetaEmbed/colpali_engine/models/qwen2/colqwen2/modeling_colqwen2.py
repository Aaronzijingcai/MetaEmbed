...
class BidirectionalQwen2VLModel(Qwen2VLModel):
...
class ColQwen2(Qwen2VLForConditionalGeneration):
    ...
    def __init__(
        ...
        # self.model.layers -> nn.ModuleList([Qwen2VLDecoderLayer])

        if use_bidirectional_attention:
            rank0_print("Using bidirectional attention for ColPali Qwen2 instance.")
            self.model = BidirectionalQwen2VLModel(config=config)

        if use_liger_kernel:
            rank0_print("Applying Liger kernel to ColPali Qwen2 instance.")
            apply_liger_kernel_to_qwen2_vl(self)
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        # Handle the custom "pixel_values" input obtained with
        # `ColQwen2Processor` through unpadding
