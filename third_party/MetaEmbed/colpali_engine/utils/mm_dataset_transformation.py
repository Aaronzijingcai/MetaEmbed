    ...
    def __call__(self):
        ...
        for subset, meta_info in self.subset2meta.items():
            if "num_samples" in meta_info:
                ...
                ds = load_dataset(
                    base_path + ds_path, subset, num_proc=64, split=self.split
                )
    ...
    def __call__(self):
        ...
        for subset_id, (subset, meta_info) in enumerate(self.subset2meta.items()):
            ...
            if self.interleaved_batch_size > 0:
                ...
                ds = ds.shuffle(buffer_size=10_000, seed=42 + subset_id)
        ...
        if len(ds_list) > 1:
            ...
            if self.interleaved_batch_size > 0:
                train_ds = interleave_datasets(
                    ...
                    seed=42
                    + dist.get_rank(),  # ensure the data source sampling is different across GPUs
