...
From https://github.com/TIGER-AI-Lab/VLM2Vec/blob/main/src/data/dataset/hf_datasets.py
"""
...
def interleave_datasets(
    ...
    stopping_strategy: Literal[
        "first_exhausted", "all_exhausted", "never_exhausted"
    ] = "first_exhausted",
) -> DatasetType:
    """
    ...
    if stopping_strategy not in ["first_exhausted", "all_exhausted", "never_exhausted"]:
        raise ValueError(
            f"{stopping_strategy} is not supported. Please enter a valid stopping_strategy."
        )
    if dataset_type is Dataset:
        raise NotImplementedError(
            f"Interleaving {dataset_type.__name__} is not supported yet."
        )
    else:
...
def _interleave_iterable_datasets(
    ...
    # shuffling: Optional[ShufflingConfig] = None,  # we should keep
) -> IterableDataset:
    """
    ...
    for idx, d in enumerate(datasets):
        rank0_print(f"\t\tDataset {idx} features: {[f for f in d.features]}")
    rank0_print("=" * 50)
    ...
    rank0_print(
        f"Check the features of the new dataset: {features}",
    )
    ...
    if batch_size is not None and batch_size > 0:
        ...
        if probabilities is None:
        ...
        else:  # We mainly use this option
            generator = np.random.default_rng(seed)
            # WHETHER to call shuffle_data_source here is a question
            ...
            # print(f"Shuffling data sources with probs {probabilities}!!!!!")
    else:
...
class CyclingMultiSourcesBatchesIterable(_BaseExamplesIterable):
    def __init__(
        ...
        # self.bool_strategy_func = lambda _: (
        #     False if stopping_strategy == "never_exhausted" else self.bool_strategy_func
        # )
    ...
    def __iter__(self):
        ...
        for i in indices_iterator:
            ...
            try:  # let's pick one example from the iterator at index i
                # if count % 100 == 0:
                #     rank0_print(f"count: {count}, i: {i}")
                # why is this getting reset?
...
class RandomlyCyclingMultiSourcesBatchesIterable(CyclingMultiSourcesBatchesIterable):
    ...
    def _get_indices_iterator(self):
        ...
        rng = self.generator  # this makes the rng used across epochs
    ...
    def shuffle_data_sources(
        ...
        # external generator was epoch dependent, which ensures the sampling is different across epochs
        ex_iterables = [
            ex_iterable.shuffle_data_sources(generator)
            for ex_iterable in self.ex_iterables
        ]
