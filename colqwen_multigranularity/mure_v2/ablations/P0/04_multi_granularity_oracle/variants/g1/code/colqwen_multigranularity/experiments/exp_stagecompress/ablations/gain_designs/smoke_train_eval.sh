...
RUN_NAME=${RUN_NAME:-folder_gain_only_v1_${GAIN_MODE}_smoke_$(date +%Y%m%d_%H%M%S)}
...
export DOC_CHUNK_SIZE=${DOC_CHUNK_SIZE:-8}
export QUERY_CHUNK_SIZE=${QUERY_CHUNK_SIZE:-16}
export MAX_NUM_VISUAL_TOKENS=${MAX_NUM_VISUAL_TOKENS:-512}
export WANDB_MODE=${WANDB_MODE:-offline}
export DDP_FIND_UNUSED_PARAMETERS=${DDP_FIND_UNUSED_PARAMETERS:-0}
export GRADIENT_CHECKPOINTING=${GRADIENT_CHECKPOINTING:-1}

echo "[gain_smoke] train run_dir=$RUN_DIR mode=$GAIN_MODE devices=$CUDA_DEVICE_LIST num_gpus=$NUM_GPUS max_steps=$MAX_STEPS"
...
if [[ -z "${CHECKPOINT:-}" || ! -d "$CHECKPOINT" ]]; then
  echo "[gain_smoke] no checkpoint found under $RUN_DIR" >&2
  exit 2
fi
if [[ ! -f "$CHECKPOINT/folder_gain_only.pt" ]]; then
  echo "[gain_smoke] folder_gain_only.pt not found under $CHECKPOINT" >&2
  exit 2
fi
...
export OUT_DIR=${OUT_DIR:-$RUN_DIR/eval/gain_smoke_${GAIN_MODE}}
export LOG_FILE=${EVAL_LOG_FILE:-$RUN_DIR/logs/eval_gain_smoke_$(date +%Y%m%d_%H%M%S).log}

echo "[gain_smoke] eval checkpoint=$CHECKPOINT out_dir=$OUT_DIR"
bash "$SCRIPT_DIR/eval_3sets.sh" "$CHECKPOINT"

echo "[gain_smoke] done"
echo "[gain_smoke] run_dir=$RUN_DIR"
echo "[gain_smoke] checkpoint=$CHECKPOINT"
echo "[gain_smoke] eval_dir=$OUT_DIR"
