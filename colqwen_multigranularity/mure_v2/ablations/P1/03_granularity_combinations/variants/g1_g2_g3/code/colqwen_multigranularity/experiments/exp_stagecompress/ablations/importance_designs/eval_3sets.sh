...
CHECKPOINT=${1:-${CHECKPOINT:-$SCRIPT_DIR/runs/folder_importance_v1_${IMPORTANCE_MODE}_b160_160_160_4k/checkpoint-4000}}
MODEL_RUN_DIR=$(cd "$(dirname "$CHECKPOINT")" && pwd)
OUT_DIR=${OUT_DIR:-$MODEL_RUN_DIR/eval/folder_importance_${IMPORTANCE_MODE}}
LOG_DIR=${LOG_DIR:-$MODEL_RUN_DIR/logs}
LOG_FILE=${LOG_FILE:-$LOG_DIR/eval_importance_$(date +%Y%m%d_%H%M%S).log}
...
if [[ ! -f "$CHECKPOINT/folder_importance.pt" ]]; then
  echo "folder_importance.pt not found under checkpoint: $CHECKPOINT" >&2
  exit 2
fi
...
COMMON_ARGS=(
  ...
  --importance-eval-prefix-level "$EVAL_PREFIX_LEVEL"
)

{
  echo "[importance_eval] $(date +%Y-%m-%d\ %H:%M:%S) starting eval"
  echo "[importance_eval] CHECKPOINT=$CHECKPOINT OUT_DIR=$OUT_DIR"
  echo "[importance_eval] BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES IMPORTANCE_MODE=$IMPORTANCE_MODE IMPORTANCE_BLEND=$IMPORTANCE_BLEND"
  echo "[importance_eval] EVAL_MODE=$EVAL_MODE EVAL_MAX_QUERIES=$EVAL_MAX_QUERIES EVAL_MAX_CORPUS=$EVAL_MAX_CORPUS NUM_WORKERS=$NUM_WORKERS"
}
...
run_eval() {
  ...
  shift 3
  local keywords=("$@")
  echo "[importance_eval] run=$name format=$format avg_metric=$avg_metric keywords=${keywords[*]}"
