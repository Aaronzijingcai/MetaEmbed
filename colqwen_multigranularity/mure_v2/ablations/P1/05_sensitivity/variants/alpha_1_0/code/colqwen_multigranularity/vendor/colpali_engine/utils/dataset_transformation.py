...
def load_nq_train_set() -> DatasetDict:
    ds_path = "natural-questions"
    base_path = "./data_dir/" if USE_LOCAL_DATASET else "sentence-transformers/"
    ds_dict = cast(DatasetDict, load_dataset(base_path + ds_path, split="train"))
    return ds_dict
...
def load_train_set_local() -> DatasetDict:
    ds_path = "colpali_train_set"
    base_path = os.environ.get("DATA_DIR", "./data_dir/") if USE_LOCAL_DATASET else "vidore/"
    ds_dict = cast(DatasetDict, load_dataset(base_path + ds_path, num_proc=128))
    return ds_dict
...
# this docmatix dataset is w/ negatives
def load_docmatix_ir_negs() -> Tuple[DatasetDict, Dataset, str]:
    ...
    dataset = cast(
        Dataset, load_dataset(base_path + "docmatix-ir", split="train", num_proc=128)
    )
    ...
    anchor_ds = cast(
        Dataset,
        load_dataset(base_path + "Docmatix", "images", split="train", num_proc=128),
    )
...
# this docmatix dataset is w/ negatives
def load_docmatix_ir_negs_local() -> Tuple[DatasetDict, Dataset, str]:
    """Returns the query dataset, then the anchor dataset with the documents, then the dataset type"""
    ...
    base_path = (
        os.environ.get("DATA_DIR", "./data_dir/") if USE_LOCAL_DATASET else "HuggingFaceM4/"
    )
    anchor_ds = cast(
        Dataset,
        load_dataset(base_path + "Docmatix", split="train", num_proc=128),
    )
...
class TestSetFactory:
    ...
    def __call__(self, *args, **kwargs):
        dataset = load_dataset(self.dataset_path, split="test", num_proc=128)
        return dataset
    ...
    def __call__(self, *args, **kwargs):
        ...
        dataset = {
            "corpus": cast(
                Dataset,
                load_dataset(
                    self.dataset_path, name="corpus", split=split, num_proc=128
                ),
            ),
            "queries": cast(
                Dataset,
                load_dataset(
                    self.dataset_path, name="queries", split=split, num_proc=128
                ),
            ),
            "qrels": cast(
                Dataset,
                load_dataset(
                    self.dataset_path, name="qrels", split=split, num_proc=128
                ),
            ),
        }
