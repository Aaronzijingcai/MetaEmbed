...
ABLATION_KIND=${ABLATION_KIND:?ABLATION_KIND must be gain or importance}
ABLATION_MODE=${ABLATION_MODE:?ABLATION_MODE is required}
...
case "$ABLATION_KIND" in
  gain)
    TRAIN_MODULE=colqwen_multigranularity.experiments.exp_stagecompress.ablations.gain_designs.train_gain
    EXTRA_ARGS=(
      --gain-enabled
      --gain-compress-stages "$COMPRESS_STAGES"
      --gain-budgets "${BUDGETS[@]}"
      --gain-mode "$ABLATION_MODE"
    ...
    ;;
  importance)
    TRAIN_MODULE=colqwen_multigranularity.experiments.exp_stagecompress.ablations.importance_designs.train_importance
    EXTRA_ARGS=(
      --importance-enabled
      --importance-compress-stages "$COMPRESS_STAGES"
      --importance-budgets "${BUDGETS[@]}"
      --importance-mode "$ABLATION_MODE"
    ...
    ;;
  *) echo "Unsupported ABLATION_KIND=$ABLATION_KIND" >&2; exit 2 ;;
esac
...
{
  echo "[mmeb_full_train] $(date +%Y-%m-%d\ %H:%M:%S) starting isolated score-design ablation"
  ...
  echo "[mmeb_full_train] ABLATION_KIND=$ABLATION_KIND ABLATION_MODE=$ABLATION_MODE BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES GRANULARITY_LOSS_WEIGHTS=${GRANULARITY_LOSS_WEIGHTS[*]}"
