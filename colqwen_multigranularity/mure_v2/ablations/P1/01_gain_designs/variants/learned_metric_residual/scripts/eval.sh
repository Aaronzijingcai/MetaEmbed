...
case "$ABLATION_KIND" in
  gain)
    EVAL_MODULE=colqwen_multigranularity.experiments.exp_stagecompress.ablations.gain_designs.eval_gain
    STATE_FILE=folder_gain_only.pt
    DESIGN_ARGS=(
      --gain-enabled
    ...
    ;;
  importance)
    EVAL_MODULE=colqwen_multigranularity.experiments.exp_stagecompress.ablations.importance_designs.eval_importance
    STATE_FILE=folder_importance.pt
    DESIGN_ARGS=(
      --importance-enabled
...
if [[ ! -f "$CHECKPOINT/$STATE_FILE" ]]; then
  echo "Missing score-design state: $CHECKPOINT/$STATE_FILE" >&2
  exit 2
fi
