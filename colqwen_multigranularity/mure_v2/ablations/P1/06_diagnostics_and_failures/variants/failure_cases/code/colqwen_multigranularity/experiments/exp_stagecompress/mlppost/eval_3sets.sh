...
CHECKPOINT=${1:-${CHECKPOINT:-$PROJECT_DIR/experiments/exp_stagecompress/runs/stagecompress_8gpu_all_${METHOD}_nommE5_textquery_focus_4k/checkpoint-4000}}
MODEL_RUN_DIR=$(cd "$(dirname "$CHECKPOINT")" && pwd)
OUT_DIR=${OUT_DIR:-$MODEL_RUN_DIR/eval/${METHOD}}
...
if [[ ! -f "$CHECKPOINT/stage_compressor.pt" ]]; then
  echo "stage_compressor.pt not found under checkpoint: $CHECKPOINT" >&2
  exit 2
fi
...
{
  echo "[eval_3sets] $(date +%Y-%m-%d\ %H:%M:%S) starting StageCompress eval"
  echo "[eval_3sets] METHOD=$METHOD CHECKPOINT=$CHECKPOINT"
  echo "[eval_3sets] OUT_DIR=$OUT_DIR LOG_FILE=$LOG_FILE"
  ...
  echo "[eval_3sets] COMPRESS_STAGES=$COMPRESS_STAGES BUDGETS=${BUDGETS[*]} BEIR_AVG_METRIC=$BEIR_AVG_METRIC MMEB_AVG_METRIC=$MMEB_AVG_METRIC"
...
run_eval() {
  ...
  shift 3
  local keywords=("$@")
  echo "[eval_3sets] run=$name format=$format avg_metric=$avg_metric keywords=${keywords[*]}"
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.mlppost.eval_stagecompress \
