...
if [[ ! -f "$CHECKPOINT/folder_homo.pt" ]]; then
  echo "Missing RHC checkpoint state: $CHECKPOINT/folder_homo.pt" >&2
  exit 2
fi
...
CUDA_VISIBLE_DEVICES="$CUDA_DEVICE_LIST" PYTHONUNBUFFERED=1 \
  ...
  --module colqwen_multigranularity.experiments.exp_stagecompress.folder_homo.eval_folder_homo \
