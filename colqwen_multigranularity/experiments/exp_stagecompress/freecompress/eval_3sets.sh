...
COMPRESS_STAGES=${COMPRESS_STAGES:-g2g3}
KEEP_RATIOS=(${KEEP_RATIOS:-1.0 0.5 0.25})
STAGE_BUDGETS=(${STAGE_BUDGETS:-0 0 0})
...
OUT_DIR=${OUT_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/runs/freecompress_${METHOD}_mrlmain/eval}
LOG_DIR=${LOG_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/runs/freecompress_${METHOD}_mrlmain/logs}
LOG_FILE=${LOG_FILE:-$LOG_DIR/eval_${METHOD}_$(date +%Y%m%d_%H%M%S).log}
...
if [[ "${#KEEP_RATIOS[@]}" -ne 3 ]]; then
  echo "KEEP_RATIOS must contain exactly 3 floats, got: ${KEEP_RATIOS[*]}" >&2
  exit 2
fi
if [[ "${#STAGE_BUDGETS[@]}" -ne 3 ]]; then
  echo "STAGE_BUDGETS must contain exactly 3 ints, got: ${STAGE_BUDGETS[*]}" >&2
  exit 2
fi
...
COMMON_ARGS=(
  ...
  --freecompress-method "$METHOD"
  --freecompress-compress-stages "$COMPRESS_STAGES"
  --freecompress-keep-ratios "${KEEP_RATIOS[@]}"
  --freecompress-stage-budgets "${STAGE_BUDGETS[@]}"
  --smoke-eval-max-queries "$EVAL_MAX_QUERIES"
  --smoke-eval-max-corpus "$EVAL_MAX_CORPUS"
)

{
  echo "[freecompress_eval] $(date +%Y-%m-%d\ %H:%M:%S) starting"
  echo "[freecompress_eval] METHOD=$METHOD CHECKPOINT=$CHECKPOINT"
  echo "[freecompress_eval] COMPRESS_STAGES=$COMPRESS_STAGES KEEP_RATIOS=${KEEP_RATIOS[*]} STAGE_BUDGETS=${STAGE_BUDGETS[*]}"
  echo "[freecompress_eval] CUDA_DEVICE_LIST=$CUDA_DEVICE_LIST NUM_GPUS=$NUM_GPUS MAIN_PROCESS_PORT=$MAIN_PROCESS_PORT"
  echo "[freecompress_eval] EVAL_MODE=$EVAL_MODE EVAL_MAX_QUERIES=$EVAL_MAX_QUERIES EVAL_MAX_CORPUS=$EVAL_MAX_CORPUS"
  echo "[freecompress_eval] OUT_DIR=$OUT_DIR LOG_FILE=$LOG_FILE"
}
...
run_eval() {
  ...
  shift 3
  local keywords=("$@")
  echo "[freecompress_eval] run=$name format=$format avg_metric=$avg_metric keywords=${keywords[*]}"
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.freecompress.eval_freecompress \
    "${COMMON_ARGS[@]}" \
