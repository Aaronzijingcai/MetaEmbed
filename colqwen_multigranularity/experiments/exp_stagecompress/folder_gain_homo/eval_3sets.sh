...
GAIN_MODE=${GAIN_MODE:-basic}
NOVELTY_WEIGHT=${NOVELTY_WEIGHT:-1.0}
COVERAGE_WEIGHT=${COVERAGE_WEIGHT:-0.5}
MMR_WEIGHT=${MMR_WEIGHT:-0.5}
RESIDUAL_MASS_WEIGHT=${RESIDUAL_MASS_WEIGHT:-0.25}
RESIDUAL_MASS_MIN_BUDGET_RATIO=${RESIDUAL_MASS_MIN_BUDGET_RATIO:-0.5}
RESIDUAL_MASS_TOPK_RATIO=${RESIDUAL_MASS_TOPK_RATIO:-0.25}
GEO_RADIUS=${GEO_RADIUS:-0.35}
GEO_TWO_CROP_LAYOUT=${GEO_TWO_CROP_LAYOUT:-vertical}
...
CHECKPOINT=${1:-${CHECKPOINT:-$PROJECT_DIR/experiments/exp_stagecompress/runs/folder_gain_homo_8gpu_all_nommE5_textquery_focus_4k/checkpoint-4000}}
MODEL_RUN_DIR=$(cd "$(dirname "$CHECKPOINT")" && pwd)
OUT_DIR=${OUT_DIR:-$MODEL_RUN_DIR/eval/folder_gain_homo}
LOG_DIR=${LOG_DIR:-$MODEL_RUN_DIR/logs}
LOG_FILE=${LOG_FILE:-$LOG_DIR/eval_folder_gain_homo_$(date +%Y%m%d_%H%M%S).log}
...
if [[ ! -f "$CHECKPOINT/folder_gain_homo.pt" ]]; then
  echo "folder_gain_homo.pt not found under checkpoint: $CHECKPOINT" >&2
  exit 2
fi
...
COMMON_ARGS=(
  --folder-gain-homo-enabled
  --folder-gain-homo-compress-stages "$COMPRESS_STAGES"
  --folder-gain-homo-budgets "${BUDGETS[@]}"
  --folder-gain-homo-mode "$GAIN_MODE"
  --folder-gain-homo-novelty-weight "$NOVELTY_WEIGHT"
  --folder-gain-homo-coverage-weight "$COVERAGE_WEIGHT"
  --folder-gain-homo-mmr-weight "$MMR_WEIGHT"
  --folder-gain-homo-residual-mass-weight "$RESIDUAL_MASS_WEIGHT"
  --folder-gain-homo-residual-mass-min-budget-ratio "$RESIDUAL_MASS_MIN_BUDGET_RATIO"
  --folder-gain-homo-residual-mass-topk-ratio "$RESIDUAL_MASS_TOPK_RATIO"
  --folder-gain-homo-geo-radius "$GEO_RADIUS"
  --folder-gain-homo-geo-two-crop-layout "$GEO_TWO_CROP_LAYOUT"
  --folder-gain-homo-gate-strength "$GATE_STRENGTH"
  --folder-gain-homo-folder-alpha "$FOLDER_ALPHA"
  --folder-gain-homo-eval-prefix-level "$EVAL_PREFIX_LEVEL"
)

{
  echo "[folder_gain_homo_eval] $(date +%Y-%m-%d\ %H:%M:%S) starting eval"
  echo "[folder_gain_homo_eval] CHECKPOINT=$CHECKPOINT OUT_DIR=$OUT_DIR LOG_FILE=$LOG_FILE"
  echo "[folder_gain_homo_eval] CUDA_DEVICE_LIST=$CUDA_DEVICE_LIST NUM_GPUS=$NUM_GPUS MAIN_PROCESS_PORT=$MAIN_PROCESS_PORT"
  echo "[folder_gain_homo_eval] BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES GAIN_MODE=$GAIN_MODE EVAL_PREFIX_LEVEL=$EVAL_PREFIX_LEVEL"
  echo "[folder_gain_homo_eval] NOVELTY_WEIGHT=$NOVELTY_WEIGHT COVERAGE_WEIGHT=$COVERAGE_WEIGHT MMR_WEIGHT=$MMR_WEIGHT RESIDUAL_MASS_WEIGHT=$RESIDUAL_MASS_WEIGHT"
  echo "[folder_gain_homo_eval] RESIDUAL_MASS_MIN_BUDGET_RATIO=$RESIDUAL_MASS_MIN_BUDGET_RATIO RESIDUAL_MASS_TOPK_RATIO=$RESIDUAL_MASS_TOPK_RATIO GEO_RADIUS=$GEO_RADIUS GEO_TWO_CROP_LAYOUT=$GEO_TWO_CROP_LAYOUT GATE_STRENGTH=$GATE_STRENGTH FOLDER_ALPHA=$FOLDER_ALPHA"
  echo "[folder_gain_homo_eval] QUERY_AUGMENTATION_REPEATS=$QUERY_AUGMENTATION_REPEATS DOCUMENT_AUGMENTATION_REPEATS=$DOCUMENT_AUGMENTATION_REPEATS"
  echo "[folder_gain_homo_eval] MAXSIM_QUERY_DROP_PREFIX=$MAXSIM_QUERY_DROP_PREFIX MAXSIM_QUERY_DROP_SUFFIX=$MAXSIM_QUERY_DROP_SUFFIX MAXSIM_QUERY_AGG=$MAXSIM_QUERY_AGG MAXSIM_QUERY_TOPK=$MAXSIM_QUERY_TOPK MAXSIM_LENGTH_NORM_ALPHA=$MAXSIM_LENGTH_NORM_ALPHA MAXSIM_HIT_PENALTY_WEIGHT=$MAXSIM_HIT_PENALTY_WEIGHT MAXSIM_HIT_PENALTY_THRESHOLD=$MAXSIM_HIT_PENALTY_THRESHOLD"
}
...
run_eval() {
  ...
  shift 3
  local keywords=("$@")
  echo "[folder_gain_homo_eval] run=$name format=$format avg_metric=$avg_metric keywords=${keywords[*]}"
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.folder_gain_homo.eval_folder_gain_homo \
...
echo "[folder_gain_homo_eval] done"
