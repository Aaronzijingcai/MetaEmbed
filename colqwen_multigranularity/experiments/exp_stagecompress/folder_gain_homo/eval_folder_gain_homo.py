...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--folder-gain-homo-eval-prefix-level', type=int, default=3)
...
def build_config(args: argparse.Namespace) -> FolderGainHomoConfig:
    return FolderGainHomoConfig(
        ...
        eval_prefix_level=int(args.folder_gain_homo_eval_prefix_level),
    )


def build_model(args: argparse.Namespace, folder_gain_homo_config: FolderGainHomoConfig):
    ...
    folder_gain_homo_dir: Optional[Path] = None
    if args.adapter_path is not None:
        folder_gain_homo_dir = Path(args.adapter_path)
    elif args.mrl_state_dict_path is not None:
        folder_gain_homo_dir = Path(args.mrl_state_dict_path).parent
    ...
    if folder_gain_homo_dir is not None:
        extra_state = folder_gain_homo_dir / 'folder_gain_homo.pt'
...
def main() -> None:
    ...
    model = build_model(args, folder_gain_homo_config)
    if torch.cuda.is_available():
    ...
    if is_rank0:
        print(json.dumps({
            'event': 'folder_gain_homo_eval_start',
            'gain_mode': folder_gain_homo_config.normalized_gain_mode(),
            'compress_stages': folder_gain_homo_config.compress_stages,
            'budgets': folder_gain_homo_config.budgets,
            'eval_prefix_level': folder_gain_homo_config.eval_prefix_level,
            'novelty_weight': folder_gain_homo_config.novelty_weight,
            'coverage_weight': folder_gain_homo_config.coverage_weight,
            'mmr_weight': folder_gain_homo_config.mmr_weight,
            'residual_mass_weight': folder_gain_homo_config.residual_mass_weight,
            'residual_mass_min_budget_ratio': folder_gain_homo_config.residual_mass_min_budget_ratio,
            'residual_mass_topk_ratio': folder_gain_homo_config.residual_mass_topk_ratio,
            'geo_radius': folder_gain_homo_config.geo_radius,
            'geo_two_crop_layout': folder_gain_homo_config.geo_two_crop_layout,
            'gate_strength': folder_gain_homo_config.gate_strength,
