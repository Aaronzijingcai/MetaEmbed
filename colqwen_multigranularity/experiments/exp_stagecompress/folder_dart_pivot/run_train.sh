...
RUN_NAME=${RUN_NAME:-folder_dart_pivot_8gpu_all_nommE5_textquery_focus_4k}
...
if [[ "$TRAIN_COMPRESSOR_ONLY" == "1" || "$TRAIN_COMPRESSOR_ONLY" == "true" || "$TRAIN_COMPRESSOR_ONLY" == "TRUE" ]]; then
  EXTRA_ARGS+=(--folder-dart-pivot-train-compressor-only)
fi
...
{
  echo "[folder_dart_pivot_launcher] $(date +%Y-%m-%d\ %H:%M:%S) starting FolderDartPivot training"
  echo "[folder_dart_pivot_launcher] OUTPUT_DIR=$OUTPUT_DIR LOG_FILE=$LOG_FILE"
  echo "[folder_dart_pivot_launcher] CUDA_DEVICE_LIST=$CUDA_DEVICE_LIST NUM_GPUS=$NUM_GPUS MAIN_PROCESS_PORT=$MAIN_PROCESS_PORT"
  echo "[folder_dart_pivot_launcher] BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES MAX_STEPS=$MAX_STEPS SAVE_STEPS=$SAVE_STEPS"
  echo "[folder_dart_pivot_launcher] TRAIN_BSZ=$TRAIN_BSZ EVAL_BSZ=$EVAL_BSZ INTERLEAVED_BSZ=$INTERLEAVED_BSZ GRAD_ACCUM_STEPS=$GRAD_ACCUM_STEPS"
  echo "[folder_dart_pivot_launcher] MODEL_PATH=$MODEL_PATH USE_PEFT=$USE_PEFT TRAIN_COMPRESSOR_ONLY=$TRAIN_COMPRESSOR_ONLY GRADIENT_CHECKPOINTING=$GRADIENT_CHECKPOINTING"
  echo "[folder_dart_pivot_launcher] NOVELTY_WEIGHT=$NOVELTY_WEIGHT PIVOT_COUNT=$PIVOT_COUNT PIVOT_SCORE=$PIVOT_SCORE GATE_STRENGTH=$GATE_STRENGTH FOLDER_ALPHA=$FOLDER_ALPHA"
} >> "$LOG_FILE"
...
"$ACCELERATE_BIN" launch \
  ...
  -m colqwen_multigranularity.experiments.exp_stagecompress.folder_dart_pivot.train_folder_dart_pivot \
