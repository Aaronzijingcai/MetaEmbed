...
from .loss import FolderDartPivotMRLInBatchNegativeLoss
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--folder-dart-pivot-skip-save', action='store_true', default=False)
    parser.add_argument('--folder-dart-pivot-train-compressor-only', action='store_true', default=False)
...
def main() -> None:
    ...
    try:
        ...
        t0 = time.time()
        base_train.logger.info('Building FolderDartPivot processor...')
        processor = base_train.build_processor(args)
        base_train.logger.info('Processor built in %.1fs', time.time() - t0)

        t0 = time.time()
        base_train.logger.info('Building FolderDartPivot model...')
        ...
        loss_func = FolderDartPivotMRLInBatchNegativeLoss(
        ...
        if args.folder_dart_pivot_train_compressor_only:
            for param in config.model.parameters():
                param.requires_grad = False

        if args.use_peft or args.folder_dart_pivot_train_compressor_only:
            for name, param in config.model.named_parameters():
                if 'folder_dart_pivot' in name:
        ...
        base_train.logger.info("FolderDartPivot final trainable params: %d || all params: %d || trainable%%: %.6f", trainable_params, total_params, 100.0 * trainable_params / max(total_params, 1))

        def _get_folder_dart_pivot(module):
            for name, submodule in module.named_modules():
                if name.endswith('folder_dart_pivot'):
                    return submodule
            return None

        def _save_folder_dart_pivot(save_dir: Path):
            save_dir.mkdir(parents=True, exist_ok=True)
            homo = _get_folder_dart_pivot(config.model)
            if homo is not None:
                torch.save(homo.state_dict(), save_dir / 'folder_dart_pivot.pt')

        folder_dart_pivot_state_path = None
        if args.resume_from_checkpoint:
            candidate = Path(args.resume_from_checkpoint) / 'folder_dart_pivot.pt'
            if candidate.exists():
                folder_dart_pivot_state_path = candidate
        if folder_dart_pivot_state_path is not None:
            homo = _get_folder_dart_pivot(config.model)
            if homo is not None:
                homo.load_state_dict(torch.load(folder_dart_pivot_state_path, map_location='cpu'), strict=False)

        class _SaveFolderDartPivotCallback(TrainerCallback):
            def on_save(self, args, state, control, model=None, **kwargs):
                _save_folder_dart_pivot(Path(args.output_dir) / f'checkpoint-{state.global_step}')
                return control

        training_app = ColModelTraining(config, Path(__file__))
        training_app.init_trainer()
        training_app.trainer.add_callback(_SaveFolderDartPivotCallback())
        training_app.train()
        if not args.folder_dart_pivot_skip_save:
            training_app.save()
            _save_folder_dart_pivot(Path(args.output_dir))
        if args.run_eval:
            training_app.eval()
    finally:
