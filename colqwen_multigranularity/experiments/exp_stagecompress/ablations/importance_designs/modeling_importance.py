...
class ImportancePatchScorer(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if attention_mode == "received":
            # Received-attention importance only needs the column mean of the
            # attention graph. Accumulate that statistic by query blocks so
            # long visual sequences never materialize the complete N x N
            # matrix. Attention is independent across query positions, so this
            # preserves the scorer definition.
            query_chunk_size = 128
            attn_chunks = []
            received_sum = tokens.new_zeros((tokens.shape[0],))
            for start in range(0, tokens.shape[0], query_chunk_size):
                end = min(start + query_chunk_size, tokens.shape[0])
                attn_chunk, weight_chunk = self.attn(
                    x[:, start:end],
                    x,
                    x,
                    need_weights=True,
                ...
                received_sum = received_sum + weight_chunk.squeeze(0).sum(dim=0).to(tokens.dtype)
            attn_out = torch.cat(attn_chunks, dim=1)
            attn_weights = None
            received_attention = received_sum / max(int(tokens.shape[0]), 1)
            token_attention = tokens.new_zeros((0, 0))
        else:
            need_attention = attention_mode == "full"
            attn_out, attn_weights = self.attn(
                x,
                x,
                x,
                need_weights=need_attention,
                # All full-graph importance variants consume the head mean.
                # Request it directly instead of materializing one N x N
                # matrix per head.
                average_attn_weights=True,
            )
            if attn_weights is None:
                token_attention = tokens.new_zeros((0, 0))
                received_attention = tokens.new_zeros((tokens.shape[0],))
            else:
                # attn_weights: [1, target_tokens, source_tokens], already
                # averaged across heads by MultiheadAttention.
                token_attention = attn_weights.squeeze(0).to(dtype=tokens.dtype)
...
class ImportanceFolderBlock(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        full_attention_modes = {
            "mha_pagerank",
            "pagerank",
            "attention_pagerank",
            "mha_entropy_confidence",
            "attn_confidence",
            "entropy_confidence",
            "mlp_mha",
            "hybrid_mha",
        ...
        attention_mode = (
            "received"
            if mode in received_attention_modes
            else "full"
            if mode in full_attention_modes
            else "none"
        )
        enhanced, saliency, gate, received_attention, token_attention = self.scorer(
            tokens,
