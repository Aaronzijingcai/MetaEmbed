...
RUN_NAME=${RUN_NAME:-folder_homo_8gpu_all_nommE5_textquery_focus_4k}
...
{
  echo "[folder_homo_launcher] $(date +%Y-%m-%d\ %H:%M:%S) starting FolderHomo training"
  echo "[folder_homo_launcher] OUTPUT_DIR=$OUTPUT_DIR LOG_FILE=$LOG_FILE"
  echo "[folder_homo_launcher] CUDA_DEVICE_LIST=$CUDA_DEVICE_LIST NUM_GPUS=$NUM_GPUS MAIN_PROCESS_PORT=$MAIN_PROCESS_PORT"
  echo "[folder_homo_launcher] BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES MAX_STEPS=$MAX_STEPS SAVE_STEPS=$SAVE_STEPS"
  echo "[folder_homo_launcher] TRAIN_BSZ=$TRAIN_BSZ EVAL_BSZ=$EVAL_BSZ INTERLEAVED_BSZ=$INTERLEAVED_BSZ GRAD_ACCUM_STEPS=$GRAD_ACCUM_STEPS"
  echo "[folder_homo_launcher] MODEL_PATH=$MODEL_PATH USE_PEFT=$USE_PEFT TRAIN_COMPRESSOR_ONLY=$TRAIN_COMPRESSOR_ONLY GRADIENT_CHECKPOINTING=$GRADIENT_CHECKPOINTING"
  echo "[folder_homo_launcher] NOVELTY_WEIGHT=$NOVELTY_WEIGHT GATE_STRENGTH=$GATE_STRENGTH FOLDER_ALPHA=$FOLDER_ALPHA"
  echo "[folder_homo_launcher] MARC_ENABLED=$MARC_ENABLED MARC_MODE=$MARC_MODE MARC_WEIGHT=$MARC_WEIGHT MARC_BETA=$MARC_BETA MARC_MARGIN=$MARC_MARGIN MARC_TAU=$MARC_TAU MARC_DUP_THRESHOLD=$MARC_DUP_THRESHOLD MARC_ANCHOR_BOOST=$MARC_ANCHOR_BOOST MARC_ANCHOR_FLOOR=$MARC_ANCHOR_FLOOR"
  echo "[folder_homo_launcher] IGNORE_DATA_SKIP=$IGNORE_DATA_SKIP RESUME_CKPT=$RESUME_CKPT WARM_START_ADAPTER_PATH=$WARM_START_ADAPTER_PATH"
} >> "$LOG_FILE"
