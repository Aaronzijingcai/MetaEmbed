...
from .loss import FolderHomoMRLInBatchNegativeLoss
...
def main() -> None:
    ...
    try:
        ...
        t0 = time.time()
        base_train.logger.info('Building FolderHomo processor...')
        processor = base_train.build_processor(args)
        base_train.logger.info('Processor built in %.1fs', time.time() - t0)

        t0 = time.time()
        base_train.logger.info('Building FolderHomo model...')
        ...
        base_train.logger.info("FolderHomo final trainable params: %d || all params: %d || trainable%%: %.6f", trainable_params, total_params, 100.0 * trainable_params / max(total_params, 1))
        ...
        if folder_homo_state_path is not None:
            homo = _get_folder_homo(config.model)
            if homo is not None:
                homo.load_state_dict(torch.load(folder_homo_state_path, map_location='cpu'), strict=False)
