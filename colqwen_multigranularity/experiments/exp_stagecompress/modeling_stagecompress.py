...
class StageCompressor(nn.Module):
    def __init__(self, config: StageCompressConfig, *, image_token_id: int, crop_counts: Sequence[int], embed_dim: int) -> None:
        ...
        self.prefix_resampler_heads = max(1, min(int(config.scorer_heads), 8))
        self.prefix_resampler_cross = nn.ModuleList([
            nn.MultiheadAttention(embed_dim=embed_dim, num_heads=self.prefix_resampler_heads, dropout=float(config.scorer_dropout), batch_first=True)
            for _ in range(3)
        ])
        self.prefix_resampler_self = nn.ModuleList([
            nn.MultiheadAttention(embed_dim=embed_dim, num_heads=self.prefix_resampler_heads, dropout=float(config.scorer_dropout), batch_first=True)
            for _ in range(3)
        ])
        self.prefix_resampler_norm1 = nn.ModuleList([nn.LayerNorm(embed_dim) for _ in range(3)])
        self.prefix_resampler_norm2 = nn.ModuleList([nn.LayerNorm(embed_dim) for _ in range(3)])
        self.prefix_resampler_norm3 = nn.ModuleList([nn.LayerNorm(embed_dim) for _ in range(3)])
        self.prefix_resampler_mlp = nn.ModuleList([
            nn.Sequential(
        ...
        self.prefix_resampler_latents = nn.ParameterList([
            nn.Parameter(torch.randn(int(config.budgets[i]), embed_dim) / (embed_dim ** 0.5))
            for i in range(3)
        ])
    ...
    def _prefix_resample_stage(self, latent_idx: int, prefix_tokens: torch.Tensor) -> torch.Tensor:
        input_dtype = prefix_tokens.dtype
        compute_dtype = self.prefix_resampler_latents[latent_idx].dtype
        prefix_tokens = prefix_tokens.to(dtype=compute_dtype)
        latents = self.prefix_resampler_latents[latent_idx].unsqueeze(0).expand(1, -1, -1)
        memory = prefix_tokens.unsqueeze(0)
        cross_out, _ = self.prefix_resampler_cross[latent_idx](latents, memory, memory, need_weights=False)
        latents = self.prefix_resampler_norm1[latent_idx](latents + cross_out)
        self_out, _ = self.prefix_resampler_self[latent_idx](latents, latents, latents, need_weights=False)
        latents = self.prefix_resampler_norm2[latent_idx](latents + self_out)
        latents = self.prefix_resampler_norm3[latent_idx](latents + self.prefix_resampler_mlp[latent_idx](latents))
        return latents.squeeze(0).to(dtype=input_dtype)

    def _touch_prefix_resampler_parameters(self, output: torch.Tensor) -> torch.Tensor:
        zero = output.new_zeros(())
        modules = (
            self.prefix_resampler_cross,
            self.prefix_resampler_self,
            self.prefix_resampler_norm1,
            self.prefix_resampler_norm2,
            self.prefix_resampler_norm3,
            self.prefix_resampler_mlp,
        )
        for latent in self.prefix_resampler_latents:
            zero = zero + (latent.sum() * 0.0).to(dtype=output.dtype)
        for module_list in modules:
            for module in module_list:
                for param in module.parameters():
                    zero = zero + (param.sum() * 0.0).to(dtype=output.dtype)
        return output + zero

    def _forward_prefix_resampler(self, hidden_states: torch.Tensor, input_ids: torch.LongTensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        for row_hidden, row_ids, row_attn in zip(hidden_states, input_ids, attention_mask):
            ...
            g1, g2, g3 = self._split_stages(image_tokens)
            prefix1 = torch.cat([text_tokens, g1], dim=0)
            prefix2 = torch.cat([text_tokens, g1, g2], dim=0)
            prefix3 = torch.cat([text_tokens, g1, g2, g3], dim=0)
            c1 = self._prefix_resample_stage(0, prefix1)
            c2 = self._prefix_resample_stage(1, prefix2)
            c3 = self._prefix_resample_stage(2, prefix3)
            c1 = torch.nn.functional.normalize(c1, dim=-1)
            c2 = torch.nn.functional.normalize(c2, dim=-1)
            c3 = torch.nn.functional.normalize(c3, dim=-1)
        ...
        output = self._touch_prefix_resampler_parameters(output)
        if self.config.debug_shapes:
            print(f"[StageCompressor prefix] lengths={lengths[:4]} output={list(output.shape)}", flush=True)
        return output

    def forward(self, hidden_states: torch.Tensor, input_ids: torch.LongTensor, attention_mask: torch.Tensor) -> torch.Tensor:
        if self.config.method == "strategy7m_prefix_resampler":
            return self._forward_prefix_resampler(hidden_states, input_ids, attention_mask)
