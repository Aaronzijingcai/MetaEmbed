...
from .loss import FolderGlobalHomoMRLInBatchNegativeLoss
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--folder-global-homo-skip-save', action='store_true', default=False)
    parser.add_argument('--folder-global-homo-train-compressor-only', action='store_true', default=False)
...
def main() -> None:
    ...
    try:
        ...
        t0 = time.time()
        base_train.logger.info('Building FolderGlobalHomo processor...')
        processor = base_train.build_processor(args)
        base_train.logger.info('Processor built in %.1fs', time.time() - t0)

        t0 = time.time()
        base_train.logger.info('Building FolderGlobalHomo model...')
        ...
        loss_func = FolderGlobalHomoMRLInBatchNegativeLoss(
        ...
        if args.folder_global_homo_train_compressor_only:
            for param in config.model.parameters():
                param.requires_grad = False

        if args.use_peft or args.folder_global_homo_train_compressor_only:
            for name, param in config.model.named_parameters():
                if 'folder_global_homo' in name:
        ...
        base_train.logger.info("FolderGlobalHomo final trainable params: %d || all params: %d || trainable%%: %.6f", trainable_params, total_params, 100.0 * trainable_params / max(total_params, 1))

        def _get_folder_global_homo(module):
            for name, submodule in module.named_modules():
                if name.endswith('folder_global_homo'):
                    return submodule
            return None

        def _save_folder_global_homo(save_dir: Path):
            save_dir.mkdir(parents=True, exist_ok=True)
            homo = _get_folder_global_homo(config.model)
            if homo is not None:
                torch.save(homo.state_dict(), save_dir / 'folder_global_homo.pt')

        folder_global_homo_state_path = None
        if args.resume_from_checkpoint:
            candidate = Path(args.resume_from_checkpoint) / 'folder_global_homo.pt'
            if candidate.exists():
                folder_global_homo_state_path = candidate
        if folder_global_homo_state_path is not None:
            homo = _get_folder_global_homo(config.model)
            if homo is not None:
                homo.load_state_dict(torch.load(folder_global_homo_state_path, map_location='cpu'), strict=False)

        class _SaveFolderGlobalHomoCallback(TrainerCallback):
            def on_save(self, args, state, control, model=None, **kwargs):
                _save_folder_global_homo(Path(args.output_dir) / f'checkpoint-{state.global_step}')
                return control

        training_app = ColModelTraining(config, Path(__file__))
        training_app.init_trainer()
        training_app.trainer.add_callback(_SaveFolderGlobalHomoCallback())
        training_app.train()
        if not args.folder_global_homo_skip_save:
            training_app.save()
            _save_folder_global_homo(Path(args.output_dir))
        if args.run_eval:
            training_app.eval()
    finally:
