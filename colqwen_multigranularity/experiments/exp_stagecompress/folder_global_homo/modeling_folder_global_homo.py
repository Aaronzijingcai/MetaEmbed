...
class GlobalHomoPatchScorer(nn.Module):
...
class GlobalGuidedFolderBlock(nn.Module):
    def __init__(self, embed_dim: int, budget: int, *, config: FolderGlobalHomoConfig) -> None:
        super().__init__()
        self.budget = int(budget)
        self.config = config
        self.scorer = GlobalHomoPatchScorer(
            embed_dim,
    ...
    def _folder_reduce(self, tokens: torch.Tensor, enhanced: torch.Tensor, protect: torch.Tensor, *, budget: Optional[int] = None) -> torch.Tensor:
        if tokens.ndim != 2:
            raise ValueError(f'GlobalHomoFolder expects rank-2 tokens, got {tuple(tokens.shape)}')
...
class GlobalGuidedHomoCompressor(nn.Module):
    def __init__(self, config: FolderGlobalHomoConfig, *, image_token_id: int, crop_counts: Sequence[int], embed_dim: int) -> None:
        super().__init__()
        self.config = config
        ...
        if len(self.crop_counts) != 3:
            raise ValueError('GlobalGuidedHomo expects exactly three crop stages.')
        if len(config.budgets) != 3:
            raise ValueError(f'FolderGlobalHomo budgets must contain three values, got {config.budgets!r}')
        self.blocks = nn.ModuleList([
            GlobalGuidedFolderBlock(embed_dim=embed_dim, budget=int(config.budgets[index]), config=config)
    ...
    def forward(self, hidden_states: torch.Tensor, input_ids: torch.LongTensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.debug_shapes:
            print(f'[GlobalGuidedHomoCompressor] rows={debug_rows[:4]} output={list(output.shape)}', flush=True)
        return output


class GlobalGuidedHomoMRLColQwen2_5(MRLColQwen2_5):
    def __init__(self, base_model: ColQwen2_5, *, granularities: Sequence[int] = (1, 2, 4), compact_query_tokens: bool = True, folder_global_homo_config: Optional[FolderGlobalHomoConfig] = None) -> None:
        super().__init__(base_model=base_model, granularities=granularities, compact_query_tokens=compact_query_tokens)
        self.folder_global_homo_config = folder_global_homo_config or FolderGlobalHomoConfig(enabled=False)
        self.folder_global_homo = GlobalGuidedHomoCompressor(
            self.folder_global_homo_config,
    ...
    def forward(self, input_ids: torch.LongTensor, attention_mask: torch.Tensor, pixel_values: Optional[torch.Tensor] = None, image_grid_thw: Optional[torch.LongTensor] = None, **kwargs) -> torch.Tensor:
        ...
        if (not self.folder_global_homo_config.enabled) or len(self.folder_global_homo_config.active_stage_ids()) == 0:
            return self._compact_doc_embeddings(hidden_states, input_ids, attention_mask)
        return self.folder_global_homo(hidden_states, input_ids, attention_mask)
...
def _load_adapter_with_fallback(base_model: ColQwen2_5, adapter_path: Path):
    ...
    for key, value in state_dict.items():
        ...
        remapped[key] = value
    with TemporaryDirectory(prefix='folder_global_homo_eval_adapter_') as tmpdir:
...
def build_folder_global_homo_model(model_name_or_path: str, *, granularities: Sequence[int] = (1, 2, 4), folder_global_homo_config: Optional[FolderGlobalHomoConfig] = None, attn_implementation: Optional[str] = 'flash_attention_2', use_liger_kernel: bool = False, torch_dtype: torch.dtype = torch.bfloat16, adapter_path: Optional[str] = None, eval_mode: bool = False, compact_query_tokens: bool = True):
    granularities = normalize_granularities(granularities)
    if len(build_stage_specs(granularities)) != 3:
        raise ValueError('FolderGlobalHomo expects exactly three stages.')
    ...
    model = GlobalGuidedHomoMRLColQwen2_5(
