...
CHECKPOINT=${1:-${CHECKPOINT:-$PROJECT_DIR/experiments/exp_stagecompress/runs/folder_global_homo_8gpu_all_nommE5_textquery_focus_4k/checkpoint-4000}}
MODEL_RUN_DIR=$(cd "$(dirname "$CHECKPOINT")" && pwd)
OUT_DIR=${OUT_DIR:-$MODEL_RUN_DIR/eval/folder_global_homo}
LOG_DIR=${LOG_DIR:-$MODEL_RUN_DIR/logs}
LOG_FILE=${LOG_FILE:-$LOG_DIR/eval_folder_global_homo_$(date +%Y%m%d_%H%M%S).log}
...
if [[ ! -f "$CHECKPOINT/folder_global_homo.pt" ]]; then
  echo "folder_global_homo.pt not found under checkpoint: $CHECKPOINT" >&2
  exit 2
fi
...
{
  echo "[folder_global_homo_eval] $(date +%Y-%m-%d\ %H:%M:%S) starting eval"
  echo "[folder_global_homo_eval] CHECKPOINT=$CHECKPOINT OUT_DIR=$OUT_DIR LOG_FILE=$LOG_FILE"
  echo "[folder_global_homo_eval] CUDA_DEVICE_LIST=$CUDA_DEVICE_LIST NUM_GPUS=$NUM_GPUS MAIN_PROCESS_PORT=$MAIN_PROCESS_PORT"
  echo "[folder_global_homo_eval] BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES NOVELTY_WEIGHT=$NOVELTY_WEIGHT GLOBAL_GUIDANCE_WEIGHT=$GLOBAL_GUIDANCE_WEIGHT GLOBAL_MIN_BUDGET_RATIO=$GLOBAL_MIN_BUDGET_RATIO GATE_STRENGTH=$GATE_STRENGTH"
}
...
run_eval() {
  ...
  shift 3
  local keywords=("$@")
  echo "[folder_global_homo_eval] run=$name format=$format avg_metric=$avg_metric keywords=${keywords[*]}"
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.folder_global_homo.eval_folder_global_homo \
...
echo "[folder_global_homo_eval] done"
