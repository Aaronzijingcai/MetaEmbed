...
VISIONZIP_POSITION=${VISIONZIP_POSITION:-llm_early}
VISIONZIP_MODE=${VISIONZIP_MODE:-prune}
RUN_DIR=${RUN_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/llmpre/visionzip/runs/visionzip_${VISIONZIP_POSITION}_mask_8gpu_nommE5_textquery_focus_4k}
ADAPTER_PATH=${ADAPTER_PATH:-$RUN_DIR/checkpoint-4000}
OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/visionzip_full}
LOG_FILE=${LOG_FILE:-$RUN_DIR/logs/eval_visionzip_$(date +%Y%m%d_%H%M%S).log}
...
VISIONZIP_EXIT_LAYER=${VISIONZIP_EXIT_LAYER:-2}
VISIONZIP_KEEP_RATIOS=${VISIONZIP_KEEP_RATIOS:-1.0,0.5,0.25}
VISIONZIP_DOMINANT_RATIO=${VISIONZIP_DOMINANT_RATIO:-0.65}
VISIONZIP_CONTEXTUAL_RATIO=${VISIONZIP_CONTEXTUAL_RATIO:-0.05}
VISIONZIP_TEMPERATURE=${VISIONZIP_TEMPERATURE:-0.1}
VISIONZIP_MIN_MASK_VALUE=${VISIONZIP_MIN_MASK_VALUE:-0.0}
VISIONZIP_TRAIN_PRUNE=${VISIONZIP_TRAIN_PRUNE:-0}
VISIONZIP_USE_CONTEXT=${VISIONZIP_USE_CONTEXT:-1}
...
if [[ -f "$ADAPTER_PATH/visionzip_mrl_selector.pt" ]]; then
  LOAD_ARGS+=(--visionzip-state-path "$ADAPTER_PATH")
elif [[ -f "$ADAPTER_PATH/visionzip_selector.pt" ]]; then
  LOAD_ARGS+=(--visionzip-state-path "$ADAPTER_PATH/visionzip_selector.pt")
fi
...
COMMON_ARGS=(
  ...
  --visionzip-mode "$VISIONZIP_MODE"
  --visionzip-position "$VISIONZIP_POSITION"
  --visionzip-exit-layer "$VISIONZIP_EXIT_LAYER"
  --visionzip-keep-ratios "$VISIONZIP_KEEP_RATIOS"
  --visionzip-dominant-ratio "$VISIONZIP_DOMINANT_RATIO"
  --visionzip-contextual-ratio "$VISIONZIP_CONTEXTUAL_RATIO"
  --visionzip-temperature "$VISIONZIP_TEMPERATURE"
  --visionzip-min-mask-value "$VISIONZIP_MIN_MASK_VALUE"
...
if [[ "$VISIONZIP_TRAIN_PRUNE" == "1" || "$VISIONZIP_TRAIN_PRUNE" == "true" || "$VISIONZIP_TRAIN_PRUNE" == "TRUE" ]]; then
  COMMON_ARGS+=(--visionzip-train-prune)
fi
if [[ "$VISIONZIP_USE_CONTEXT" == "0" || "$VISIONZIP_USE_CONTEXT" == "false" || "$VISIONZIP_USE_CONTEXT" == "FALSE" ]]; then
  COMMON_ARGS+=(--visionzip-no-context)
fi

{
  echo "[eval_3sets] $(date +%Y-%m-%d\ %H:%M:%S) starting VisionZip eval"
  ...
  echo "[eval_3sets] VISIONZIP_POSITION=$VISIONZIP_POSITION MODE=$VISIONZIP_MODE EXIT_LAYER=$VISIONZIP_EXIT_LAYER TRAIN_PRUNE=$VISIONZIP_TRAIN_PRUNE"
  echo "[eval_3sets] VISIONZIP_KEEP_RATIOS=$VISIONZIP_KEEP_RATIOS DOMINANT=$VISIONZIP_DOMINANT_RATIO CONTEXTUAL=$VISIONZIP_CONTEXTUAL_RATIO"
...
run_eval() {
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.llmpre.visionzip.eval_visionzip \
    "${COMMON_ARGS[@]}" \
