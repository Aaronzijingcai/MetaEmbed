...
RUN_NAME=${RUN_NAME:-visionzip_${VISIONZIP_POSITION}_${VISIONZIP_MODE}_smoke_2gpu_$(date +%Y%m%d_%H%M%S)}
...
export VISIONZIP_MODE VISIONZIP_POSITION RUN_NAME RUN_DIR MAX_STEPS SAVE_STEPS
export TRAIN_BSZ=${TRAIN_BSZ:-1}
export EVAL_BSZ=${EVAL_BSZ:-1}
export INTERLEAVED_BSZ=${INTERLEAVED_BSZ:-1}
export VISIONZIP_EXIT_LAYER=${VISIONZIP_EXIT_LAYER:-2}
export VISIONZIP_KEEP_RATIOS=${VISIONZIP_KEEP_RATIOS:-0.7,0.5,0.25}
export VISIONZIP_DEBUG=${VISIONZIP_DEBUG:-1}
export VISIONZIP_DEBUG_LIMIT=${VISIONZIP_DEBUG_LIMIT:-12}
export WANDB_MODE=${WANDB_MODE:-offline}
export DDP_FIND_UNUSED_PARAMETERS=${DDP_FIND_UNUSED_PARAMETERS:-1}
export VISIONZIP_TRAIN_PRUNE=${VISIONZIP_TRAIN_PRUNE:-0}

echo "[smoke] run_dir=$RUN_DIR"
echo "[smoke] train: position=$VISIONZIP_POSITION mode=$VISIONZIP_MODE devices=$CUDA_DEVICE_LIST num_gpus=$NUM_GPUS max_steps=$MAX_STEPS port=$MAIN_PROCESS_PORT"
...
export VISIONZIP_MODE=${EVAL_VISIONZIP_MODE:-prune}
export OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/visionzip_smoke}
...
export LOG_FILE=${EVAL_LOG_FILE:-$RUN_DIR/logs/eval_visionzip_smoke_$(date +%Y%m%d_%H%M%S).log}
