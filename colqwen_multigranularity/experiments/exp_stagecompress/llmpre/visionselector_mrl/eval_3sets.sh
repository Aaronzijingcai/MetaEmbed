...
RUN_DIR=${RUN_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/llmpre/visionselector_mrl/runs/visionselector_mrl_mask_8gpu_nommE5_textquery_focus_4k}
ADAPTER_PATH=${ADAPTER_PATH:-$RUN_DIR/checkpoint-4000}
OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/visionselector_mrl_${VISIONSELECTOR_MRL_MODE:-prune}}
LOG_FILE=${LOG_FILE:-$RUN_DIR/logs/eval_visionselector_mrl_$(date +%Y%m%d_%H%M%S).log}
...
VISIONSELECTOR_MRL_MODE=${VISIONSELECTOR_MRL_MODE:-prune}
...
if [[ -f "$ADAPTER_PATH/visionselector_mrl_selector.pt" ]]; then
  LOAD_ARGS+=(--visionselector-mrl-state-path "$ADAPTER_PATH")
fi
...
{
  echo "[eval_3sets] $(date +%Y-%m-%d\ %H:%M:%S) starting VisionSelectorMRL eval"
  ...
  echo "[eval_3sets] VISIONSELECTOR_MRL_MODE=$VISIONSELECTOR_MRL_MODE POSITION=$VISIONSELECTOR_MRL_POSITION KEEP_RATIOS=$VISIONSELECTOR_MRL_KEEP_RATIOS SCORER_HIDDEN_DIM=$VISIONSELECTOR_MRL_SCORER_HIDDEN_DIM"
...
run_eval() {
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.llmpre.visionselector_mrl.eval_visionselector_mrl \
    "${COMMON_ARGS[@]}" \
