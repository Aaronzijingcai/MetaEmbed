...
RUN_NAME=${RUN_NAME:-twigstage_${TWIGSTAGE_MODE}_8gpu_nommE5_textquery_focus_4k}
...
{
  echo "[launcher] $(date +%Y-%m-%d\ %H:%M:%S) starting TwigStage training"
  ...
  echo "[launcher] TWIGSTAGE_MODE=$TWIGSTAGE_MODE EXIT_LAYER=$TWIGSTAGE_EXIT_LAYER TRAIN_PRUNE=$TWIGSTAGE_TRAIN_PRUNE"
  echo "[launcher] TWIGSTAGE_KEEP_RATIOS=$TWIGSTAGE_KEEP_RATIOS TWIGSTAGE_TEMPERATURE=$TWIGSTAGE_TEMPERATURE"
...
"$ACCELERATE_BIN" launch \
  ...
  -m colqwen_multigranularity.experiments.exp_stagecompress.llmpre.twigstage.train_twigstage \
