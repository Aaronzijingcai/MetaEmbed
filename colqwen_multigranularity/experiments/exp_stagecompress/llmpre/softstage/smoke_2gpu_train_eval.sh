...
RUN_NAME=${RUN_NAME:-softstage_smoke_2gpu_$(date +%Y%m%d_%H%M%S)}
...
export SOFTSTAGE_DEBUG=${SOFTSTAGE_DEBUG:-1}
export SOFTSTAGE_DEBUG_LIMIT=${SOFTSTAGE_DEBUG_LIMIT:-16}
...
export OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/softstage_smoke}
...
export LOG_FILE=${EVAL_LOG_FILE:-$RUN_DIR/logs/eval_softstage_smoke_$(date +%Y%m%d_%H%M%S).log}
