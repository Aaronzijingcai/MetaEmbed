...
RUN_DIR=${RUN_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/llmpre/softstage/runs/softstage_8gpu_nommE5_textquery_focus_4k}
ADAPTER_PATH=${ADAPTER_PATH:-$RUN_DIR/checkpoint-4000}
OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/softstage_mrl_full}
LOG_FILE=${LOG_FILE:-$RUN_DIR/logs/eval_softstage_$(date +%Y%m%d_%H%M%S).log}
...
if [[ -f "$ADAPTER_PATH/softstage_selector.pt" ]]; then
  LOAD_ARGS+=(--softstage-state-path "$ADAPTER_PATH")
fi
...
{
  echo "[eval_3sets] $(date +%Y-%m-%d\ %H:%M:%S) starting SoftStage eval"
  ...
  echo "[eval_3sets] protocol=MRL_Main no_learnable_global_tokens=1"
  echo "[eval_3sets] SOFTSTAGE_KEEP_RATIOS=$SOFTSTAGE_KEEP_RATIOS SOFTSTAGE_TEMPERATURE=$SOFTSTAGE_TEMPERATURE SOFTSTAGE_MIN_MASK_VALUE=$SOFTSTAGE_MIN_MASK_VALUE"
...
run_eval() {
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.llmpre.softstage.eval_softstage \
    "${COMMON_ARGS[@]}" \
