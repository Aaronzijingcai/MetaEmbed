...
RUN_NAME=${RUN_NAME:-global_mrl_tokens_smoke_2gpu_$(date +%Y%m%d_%H%M%S)}
...
export RUN_NAME RUN_DIR MAX_STEPS SAVE_STEPS
export TRAIN_BSZ=${TRAIN_BSZ:-4}
export EVAL_BSZ=${EVAL_BSZ:-1}
export INTERLEAVED_BSZ=${INTERLEAVED_BSZ:-4}
export GLOBAL_MRL_DEBUG=${GLOBAL_MRL_DEBUG:-1}
export GLOBAL_MRL_DEBUG_LIMIT=${GLOBAL_MRL_DEBUG_LIMIT:-16}
export WANDB_MODE=${WANDB_MODE:-offline}
export DDP_FIND_UNUSED_PARAMETERS=${DDP_FIND_UNUSED_PARAMETERS:-1}

echo "[smoke] run_dir=$RUN_DIR"
echo "[smoke] train: devices=$CUDA_DEVICE_LIST num_gpus=$NUM_GPUS max_steps=$MAX_STEPS port=$MAIN_PROCESS_PORT"
...
export OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/global_mrl_tokens_smoke}
export EVAL_MODE=${EVAL_MODE:-smoke}
export EVAL_MAX_QUERIES=${EVAL_MAX_QUERIES:-8}
export EVAL_MAX_CORPUS=${EVAL_MAX_CORPUS:-32}
export BATCH_QUERY=${BATCH_QUERY:-2}
export BATCH_PASSAGE=${BATCH_PASSAGE:-2}
export BATCH_SCORE=${BATCH_SCORE:-8}
export NUM_WORKERS=${NUM_WORKERS:-0}
export LOG_FILE=${EVAL_LOG_FILE:-$RUN_DIR/logs/eval_global_mrl_tokens_smoke_$(date +%Y%m%d_%H%M%S).log}
