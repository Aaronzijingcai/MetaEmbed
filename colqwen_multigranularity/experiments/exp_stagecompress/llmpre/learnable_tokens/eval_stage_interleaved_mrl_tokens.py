...
from .modeling_stage_interleaved_mrl_tokens import build_stage_interleaved_mrl_token_model, load_stage_interleaved_mrl_token_state
from .train_stage_interleaved_mrl_tokens import _parse_mrl_groups, _parse_stage_counts, _validate_mrl_groups
...
def build_model(args: argparse.Namespace):
    ...
    model = build_stage_interleaved_mrl_token_model(
        ...
        stage_interleaved_mrl_token_path=args.stage_interleaved_mrl_token_path,
        eval_mode=True,
    )
    ...
    if args.stage_interleaved_mrl_token_path is None and args.adapter_path is not None:
        candidate = Path(args.adapter_path) / "stage_interleaved_mrl_tokens.pt"
        if candidate.exists():
            load_stage_interleaved_mrl_token_state(model, candidate, map_location="cpu")
    return model
...
def main() -> None:
    ...
    try:
        ...
        _validate_mrl_groups(mrl_groups, num_query_tokens=sum(query_stage_counts), num_doc_tokens=sum(doc_stage_counts))
        if _is_rank0():
            print(
                "[StageInterleavedMRL eval] "
                f"adapter={args.adapter_path} eval_config={args.eval_config} format={args.dataset_format} "
                f"query_stage={query_stage_counts} doc_stage={doc_stage_counts} "
