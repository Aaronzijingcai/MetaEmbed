...
RUN_DIR=${RUN_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/llmpre/learnable_tokens/runs/global_mrl_tokens_8gpu_nommE5_textquery_focus_4k}
ADAPTER_PATH=${ADAPTER_PATH:-$RUN_DIR/checkpoint-4000}
OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/global_mrl_tokens_full}
LOG_FILE=${LOG_FILE:-$RUN_DIR/logs/eval_global_mrl_tokens_$(date +%Y%m%d_%H%M%S).log}
...
NUM_QUERY_MRL_TOKENS=${NUM_QUERY_MRL_TOKENS:-16}
NUM_DOC_MRL_TOKENS=${NUM_DOC_MRL_TOKENS:-64}
MRL_GROUPS=${MRL_GROUPS:-1,1,1.0;2,4,1.0;4,8,1.0;8,16,1.0;16,64,1.0}
...
if [[ -f "$ADAPTER_PATH/global_mrl_tokens.pt" ]]; then
  LOAD_ARGS+=(--global-mrl-token-path "$ADAPTER_PATH/global_mrl_tokens.pt")
fi
...
COMMON_ARGS=(
  ...
  --num-query-mrl-tokens "$NUM_QUERY_MRL_TOKENS"
  --num-doc-mrl-tokens "$NUM_DOC_MRL_TOKENS"
...
{
  echo "[eval_3sets] $(date +%Y-%m-%d\ %H:%M:%S) starting GlobalMRLToken eval"
  ...
  echo "[eval_3sets] QUERY_TOKENS=$NUM_QUERY_MRL_TOKENS DOC_TOKENS=$NUM_DOC_MRL_TOKENS MRL_GROUPS=$MRL_GROUPS"
...
run_eval() {
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.llmpre.learnable_tokens.eval_global_mrl_tokens \
    "${COMMON_ARGS[@]}" \
