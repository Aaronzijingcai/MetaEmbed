...
from .modeling_global_mrl_tokens import build_global_mrl_token_model, load_global_mrl_token_state, save_global_mrl_token_state
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Semicolon/space separated q,d,weight groups, e.g. '1,1,1;2,4,1'.",
    )
...
def _maybe_load_global_mrl_tokens(model, args: argparse.Namespace) -> None:
...
def main() -> None:
    ...
    try:
        ...
        t0 = time.time()
        base_train.logger.info("Building GlobalMRLToken processor...")
        ...
        base_train.logger.info(
            "Building GlobalMRLToken model (query_tokens=%d, doc_tokens=%d, groups=%s)...",
            args.num_query_mrl_tokens,
            args.num_doc_mrl_tokens,
            mrl_groups,
        )
        model = build_global_mrl_token_model(
        ...
        _maybe_load_global_mrl_tokens(model, args)
        ...
        def _save_extra(save_dir: Path) -> None:
            ...
            save_global_mrl_token_state(config.model, save_dir)

        class _SaveGlobalMRLTokensCallback(TrainerCallback):
        ...
        training_app.trainer.add_callback(_SaveGlobalMRLTokensCallback())
        training_app.train()
