...
from .modeling_visionzip_mrl import build_visionzip_mrl_model, load_visionzip_mrl_state, save_visionzip_mrl_state
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--visionzip-mrl-state-path", type=str, default=None)
    parser.add_argument("--visionzip-mode", type=str, choices=["mask", "prune"], default="mask")
    parser.add_argument("--visionzip-position", type=str, choices=["llm_early", "adapter_pre"], default="llm_early")
    parser.add_argument("--visionzip-exit-layer", type=int, default=2)
    parser.add_argument("--visionzip-keep-ratios", type=str, default="1.0,0.5,0.25")
    parser.add_argument("--visionzip-dominant-ratio", type=float, default=0.65)
    parser.add_argument("--visionzip-contextual-ratio", type=float, default=0.05)
    parser.add_argument("--visionzip-temperature", type=float, default=0.1)
    parser.add_argument("--visionzip-min-mask-value", type=float, default=0.0)
    parser.add_argument("--visionzip-train-prune", action="store_true", default=False)
    parser.add_argument("--visionzip-no-context", action="store_true", default=False)
...
def build_peft_config() -> LoraConfig:
    return LoraConfig(
        ...
        modules_to_save=["visionzip_selector", "custom_text_proj"],
    )


def _maybe_load_visionzip_mrl_state(model, args: argparse.Namespace) -> None:
    state_path = args.visionzip_mrl_state_path
    if state_path is None and args.resume_from_checkpoint:
        candidate = Path(args.resume_from_checkpoint) / "visionzip_mrl_selector.pt"
        if candidate.exists():
            state_path = str(candidate)
    if state_path:
        load_visionzip_mrl_state(model, state_path, map_location="cpu")
        base_train.logger.info("Loaded VisionZipMRL selector state from %s", state_path)
...
def main() -> None:
    ...
    try:
        ...
        keep_ratios = _parse_keep_ratios(args.visionzip_keep_ratios)
        level_weights = base_train._parse_level_weights(args.granularity_loss_weights, num_levels=len(granularities))

        t0 = time.time()
        base_train.logger.info("Building VisionZipMRL processor (MRL_Main protocol, granularities=%s)...", granularities)
        ...
        base_train.logger.info(
            "Building VisionZipMRL model (mode=%s, exit_layer=%d, keep_ratios=%s, train_prune=%s)...",
            args.visionzip_mode,
            args.visionzip_exit_layer,
            keep_ratios,
            args.visionzip_train_prune,
        )
        model = build_visionzip_mrl_model(
            ...
            visionzip_mode=args.visionzip_mode,
            visionzip_position=args.visionzip_position,
            visionzip_exit_layer=args.visionzip_exit_layer,
            visionzip_keep_ratios=keep_ratios,
            visionzip_dominant_ratio=args.visionzip_dominant_ratio,
            visionzip_contextual_ratio=args.visionzip_contextual_ratio,
            visionzip_temperature=args.visionzip_temperature,
            visionzip_min_mask_value=args.visionzip_min_mask_value,
            visionzip_train_prune=args.visionzip_train_prune,
            visionzip_use_context=not args.visionzip_no_context,
        )
        _maybe_load_visionzip_mrl_state(model, args)
        ...
        if args.use_peft:
            for name, param in config.model.named_parameters():
                if "visionzip_selector" in name:
                    param.requires_grad = True

        def _save_extra(save_dir: Path) -> None:
            save_dir.mkdir(parents=True, exist_ok=True)
            save_visionzip_mrl_state(config.model, save_dir)

        class _SaveVisionZipMRLCallback(TrainerCallback):
        ...
        training_app.trainer.add_callback(_SaveVisionZipMRLCallback())
        base_train.logger.info("Starting VisionZipMRL training (max_steps=%d, bsz=%d, keep_ratios=%s)...", args.max_steps, args.per_device_train_batch_size, keep_ratios)
        training_app.train()
        training_app.save()
