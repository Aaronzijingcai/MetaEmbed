...
RUN_DIR=${RUN_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/llmpre/visionzip_mrl/runs/visionzip_mrl_mask_8gpu_nommE5_textquery_focus_4k}
ADAPTER_PATH=${ADAPTER_PATH:-$RUN_DIR/checkpoint-4000}
OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/visionzip_mrl_${VISIONZIP_MRL_MODE:-prune}}
LOG_FILE=${LOG_FILE:-$RUN_DIR/logs/eval_visionzip_mrl_$(date +%Y%m%d_%H%M%S).log}
...
VISIONZIP_MRL_MODE=${VISIONZIP_MRL_MODE:-prune}
...
VISIONZIP_MRL_USE_CONTEXT=${VISIONZIP_MRL_USE_CONTEXT:-1}
...
if [[ -f "$ADAPTER_PATH/visionzip_mrl_selector.pt" ]]; then
  LOAD_ARGS+=(--visionzip-mrl-state-path "$ADAPTER_PATH")
fi
...
if [[ "$VISIONZIP_MRL_USE_CONTEXT" == "0" || "$VISIONZIP_MRL_USE_CONTEXT" == "false" || "$VISIONZIP_MRL_USE_CONTEXT" == "FALSE" ]]; then
  COMMON_ARGS+=(--visionzip-no-context)
fi

{
  echo "[eval_3sets] $(date +%Y-%m-%d\ %H:%M:%S) starting VisionZipMRL eval"
  ...
  echo "[eval_3sets] VISIONZIP_MRL_MODE=$VISIONZIP_MRL_MODE POSITION=$VISIONZIP_MRL_POSITION EXIT_LAYER=$VISIONZIP_MRL_EXIT_LAYER KEEP_RATIOS=$VISIONZIP_MRL_KEEP_RATIOS"
...
run_eval() {
  ...
  "$ACCELERATE_BIN" launch \
    ...
    -m colqwen_multigranularity.experiments.exp_stagecompress.llmpre.visionzip_mrl.eval_visionzip_mrl \
    "${COMMON_ARGS[@]}" \
