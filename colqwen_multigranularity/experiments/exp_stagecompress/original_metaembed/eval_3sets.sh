...
RUN_DIR=${RUN_DIR:-$PROJECT_DIR/experiments/exp_stagecompress/original_metaembed/runs/original_metaembed_singleimage_baseline_8gpu_bsz4_4k_20260601}
ADAPTER_PATH=${ADAPTER_PATH:-$RUN_DIR/checkpoint-4000}
OUTPUT_DIR=${OUTPUT_DIR:-$RUN_DIR/eval/original_metaembed_full}
LOG_FILE=${LOG_FILE:-$RUN_DIR/logs/eval_original_metaembed_$(date +%Y%m%d_%H%M%S).log}
...
NUM_QUERY_PROMPT_TOKENS=${NUM_QUERY_PROMPT_TOKENS:-16}
NUM_DOC_PROMPT_TOKENS=${NUM_DOC_PROMPT_TOKENS:-64}
...
COMMON_ARGS=(
  ...
  --num-query-prompt-tokens "$NUM_QUERY_PROMPT_TOKENS"
  --num-doc-prompt-tokens "$NUM_DOC_PROMPT_TOKENS"
...
{
  echo "[eval_3sets] $(date +%Y-%m-%d\ %H:%M:%S) starting Original MetaEmbed eval"
...
run_eval() {
  ...
  "$ACCELERATE_BIN" launch --num_machines 1 --num_processes "$NUM_GPUS" --main_process_port "$MAIN_PROCESS_PORT" --mixed_precision bf16 \
    -m colqwen_multigranularity.experiments.exp_stagecompress.original_metaembed.eval_original_metaembed \
    "${COMMON_ARGS[@]}" \
