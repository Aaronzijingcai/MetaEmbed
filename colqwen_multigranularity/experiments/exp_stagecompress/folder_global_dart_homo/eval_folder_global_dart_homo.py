...
def build_model(args: argparse.Namespace, folder_global_dart_homo_config: FolderGlobalDartHomoConfig):
    ...
    folder_global_dart_homo_dir: Optional[Path] = None
    if args.adapter_path is not None:
        folder_global_dart_homo_dir = Path(args.adapter_path)
    elif args.mrl_state_dict_path is not None:
        folder_global_dart_homo_dir = Path(args.mrl_state_dict_path).parent
    ...
    if folder_global_dart_homo_dir is not None:
        extra_state = folder_global_dart_homo_dir / 'folder_global_dart_homo.pt'
...
def main() -> None:
    ...
    model = build_model(args, folder_global_dart_homo_config)
    if torch.cuda.is_available():
    ...
    if is_rank0:
        print(json.dumps({
            'event': 'folder_global_dart_homo_eval_start',
            'compress_stages': folder_global_dart_homo_config.compress_stages,
            'budgets': folder_global_dart_homo_config.budgets,
            'novelty_weight': folder_global_dart_homo_config.novelty_weight,
            'pivot_count': folder_global_dart_homo_config.pivot_count,
            'pivot_score': folder_global_dart_homo_config.pivot_score,
            'gate_strength': folder_global_dart_homo_config.gate_strength,
