...
RUN_NAME=${RUN_NAME:-folder_global_dart_homo_8gpu_all_nommE5_textquery_focus_4k}
...
EXTRA_ARGS=(
  --folder-global-dart-homo-enabled
  --folder-global-dart-homo-compress-stages "$COMPRESS_STAGES"
  --folder-global-dart-homo-budgets "${BUDGETS[@]}"
  --folder-global-dart-homo-novelty-weight "$NOVELTY_WEIGHT"
  --folder-global-dart-homo-pivot-count "$PIVOT_COUNT"
  --folder-global-dart-homo-pivot-score "$PIVOT_SCORE"
  --folder-global-dart-homo-global-guidance-weight "$GLOBAL_GUIDANCE_WEIGHT"
  --folder-global-dart-homo-global-min-budget-ratio "$GLOBAL_MIN_BUDGET_RATIO"
  --folder-global-dart-homo-gate-strength "$GATE_STRENGTH"
  --folder-global-dart-homo-folder-alpha "$FOLDER_ALPHA"
)
...
if [[ "$TRAIN_COMPRESSOR_ONLY" == "1" || "$TRAIN_COMPRESSOR_ONLY" == "true" || "$TRAIN_COMPRESSOR_ONLY" == "TRUE" ]]; then
  EXTRA_ARGS+=(--folder-global-dart-homo-train-compressor-only)
fi
...
{
  echo "[folder_global_dart_homo_launcher] $(date +%Y-%m-%d\ %H:%M:%S) starting FolderGlobalDartHomo training"
  echo "[folder_global_dart_homo_launcher] OUTPUT_DIR=$OUTPUT_DIR LOG_FILE=$LOG_FILE"
  echo "[folder_global_dart_homo_launcher] CUDA_DEVICE_LIST=$CUDA_DEVICE_LIST NUM_GPUS=$NUM_GPUS MAIN_PROCESS_PORT=$MAIN_PROCESS_PORT"
  echo "[folder_global_dart_homo_launcher] BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES MAX_STEPS=$MAX_STEPS SAVE_STEPS=$SAVE_STEPS"
  echo "[folder_global_dart_homo_launcher] TRAIN_BSZ=$TRAIN_BSZ EVAL_BSZ=$EVAL_BSZ INTERLEAVED_BSZ=$INTERLEAVED_BSZ GRAD_ACCUM_STEPS=$GRAD_ACCUM_STEPS"
  echo "[folder_global_dart_homo_launcher] MODEL_PATH=$MODEL_PATH USE_PEFT=$USE_PEFT TRAIN_COMPRESSOR_ONLY=$TRAIN_COMPRESSOR_ONLY GRADIENT_CHECKPOINTING=$GRADIENT_CHECKPOINTING"
  echo "[folder_global_dart_homo_launcher] NOVELTY_WEIGHT=$NOVELTY_WEIGHT PIVOT_COUNT=$PIVOT_COUNT PIVOT_SCORE=$PIVOT_SCORE GLOBAL_GUIDANCE_WEIGHT=$GLOBAL_GUIDANCE_WEIGHT GLOBAL_MIN_BUDGET_RATIO=$GLOBAL_MIN_BUDGET_RATIO GATE_STRENGTH=$GATE_STRENGTH FOLDER_ALPHA=$FOLDER_ALPHA"
} >> "$LOG_FILE"
...
"$ACCELERATE_BIN" launch \
  ...
  -m colqwen_multigranularity.experiments.exp_stagecompress.folder_global_dart_homo.train_folder_global_dart_homo \
