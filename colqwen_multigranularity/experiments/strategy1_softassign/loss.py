...
from .compression import SoftAssignmentConfig
...
class SoftAssignmentMRLInBatchNegativeLoss(MRLInBatchNegativeLoss):
    ...
    def __init__(
        ...
        *,
        ...
        if len(build_stage_specs(granularities)) != 3:
            raise ValueError("Soft Assignment loss expects exactly g1/g2/g3.")
        self.strategy1_softassign_config = strategy1_softassign_config
    ...
    def _stage_lengths(self, total_image: torch.Tensor) -> torch.Tensor:
        ...
        if (not self.strategy1_softassign_config.enabled) or not self.strategy1_softassign_config.active_stage_ids():
            return original
        active = torch.tensor(
            [idx in set(self.strategy1_softassign_config.active_stage_ids()) for idx in range(3)],
            dtype=torch.bool,
            device=total_image.device,
        ).unsqueeze(0)
        has_stage = original.gt(0)
        compact = _compact_stage_lengths(original, self.strategy1_softassign_config)
    ...
    def _build_group_masks(
        ...
        *,
        ...
        # The trainer passes original ids. The model rebuilds compact rows as
        # [text_without_vision_markers; <vs> C1 <ve>; <vs> C2 <ve>; <vs> C3 <ve>],
        # so this mask mirrors that deterministic layout from original counts.
        ...
        stage_marker_lengths = image_mask.sum(dim=1).gt(0).to(torch.long).unsqueeze(1) * 2
        stage_lengths = torch.where(stage_lengths.gt(0), stage_lengths + stage_marker_lengths, stage_lengths)
    ...
    def _token_budget_stats(
        ...
        *,
        ...
        return {
            f"strategy1_softassign_{prefix}_raw_visual_tokens": (raw_visual * weights).sum() / denom,
            f"strategy1_softassign_{prefix}_compact_visual_tokens": (compact_visual * weights).sum() / denom,
            f"strategy1_softassign_{prefix}_visual_keep_ratio": (keep_ratio * weights).sum() / denom,
            f"strategy1_softassign_{prefix}_g1_tokens": (stage_lengths[:, 0] * weights).sum() / denom,
            f"strategy1_softassign_{prefix}_g2_tokens": (stage_lengths[:, 1] * weights).sum() / denom,
            f"strategy1_softassign_{prefix}_g3_tokens": (stage_lengths[:, 2] * weights).sum() / denom,
        }
    ...
    def forward(
        ...
        if query_input_ids is None or query_attention_mask is None:
            raise ValueError("query_input_ids/query_attention_mask are required for Soft Assignment MRL loss.")
        if doc_input_ids is None or doc_attention_mask is None:
            raise ValueError("doc_input_ids/doc_attention_mask are required for Soft Assignment MRL loss.")
        ...
        for level_index, (label, weight) in enumerate(zip(self.level_labels, self.level_weights)):
            ...
            loss_stats[f"strategy1_softassign_mrl_{label}"] = level_loss.detach()
            loss_stats[f"strategy1_softassign_mrl_active_ratio_{label}"] = row_mask.float().mean().detach()

        loss_stats["strategy1_softassign_query_has_images_ratio"] = query_has_images.float().mean().detach()
        loss_stats["strategy1_softassign_doc_has_images_ratio"] = pos_doc_has_images.float().mean().detach()
        ...
        if neg_doc_has_images is not None:
            loss_stats["strategy1_softassign_neg_doc_has_images_ratio"] = neg_doc_has_images.float().mean().detach()
