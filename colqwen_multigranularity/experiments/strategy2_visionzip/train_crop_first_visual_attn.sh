...
# Crop-first VisionZip with Qwen2.5-VL vision encoder attention saliency.
...
# then split by crop. Dominant tokens are selected inside each crop and emitted in original index order.
export ATTN_IMPL=${ATTN_IMPL:-eager}
export VISIONZIP_SCOPE=${VISIONZIP_SCOPE:-crop}
...
export RUN_NAME=${RUN_NAME:-strategy2_visionzip_crop_visualattn_l${VISIONZIP_VISUAL_ATTN_LAYER}_b64-128-256_$(date +%Y%m%d_%H%M%S)}
