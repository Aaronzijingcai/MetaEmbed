...
try:
    ...
    from .modeling_importance import build_importance_model
except ImportError:
    from config import FolderImportanceConfig
    from modeling_importance import build_importance_model
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--importance-eval-prefix-level", type=int, default=3)
...
def build_config(args: argparse.Namespace) -> FolderImportanceConfig:
    return FolderImportanceConfig(
        ...
        eval_prefix_level=int(args.importance_eval_prefix_level),
    )


def build_model(args: argparse.Namespace, importance_config: FolderImportanceConfig):
    ...
    importance_dir: Optional[Path] = None
    if args.adapter_path is not None:
        importance_dir = Path(args.adapter_path)
    elif args.mrl_state_dict_path is not None:
        importance_dir = Path(args.mrl_state_dict_path).parent
    ...
    if importance_dir is not None:
        extra_state = importance_dir / "folder_importance.pt"
