...
RUN_NAME=${RUN_NAME:-folder_gain_only_v1_${GAIN_MODE}_b160_160_160_4k}
...
if [[ "$TRAIN_COMPRESSOR_ONLY" == "1" || "$TRAIN_COMPRESSOR_ONLY" == "true" || "$TRAIN_COMPRESSOR_ONLY" == "TRUE" ]]; then
  EXTRA_ARGS+=(--gain-train-compressor-only)
fi
...
{
  echo "[gain_launcher] $(date +%Y-%m-%d\ %H:%M:%S) starting gain-only ablation training"
  echo "[gain_launcher] OUTPUT_DIR=$OUTPUT_DIR LOG_FILE=$LOG_FILE"
  echo "[gain_launcher] CUDA_DEVICE_LIST=$CUDA_DEVICE_LIST NUM_GPUS=$NUM_GPUS MAIN_PROCESS_PORT=$MAIN_PROCESS_PORT"
  echo "[gain_launcher] BUDGETS=${BUDGETS[*]} COMPRESS_STAGES=$COMPRESS_STAGES GAIN_MODE=$GAIN_MODE GAIN_TAU=$GAIN_TAU"
  echo "[gain_launcher] MAX_STEPS=$MAX_STEPS SAVE_STEPS=$SAVE_STEPS TRAIN_BSZ=$TRAIN_BSZ INTERLEAVED_BSZ=$INTERLEAVED_BSZ"
  echo "[gain_launcher] MODEL_PATH=$MODEL_PATH USE_PEFT=$USE_PEFT TRAIN_COMPRESSOR_ONLY=$TRAIN_COMPRESSOR_ONLY"
} >> "$LOG_FILE"
