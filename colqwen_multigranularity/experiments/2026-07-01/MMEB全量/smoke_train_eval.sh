...
RUN_NAME=${RUN_NAME:-folder_homo_mmeb_full_smoke_$(date +%Y%m%d_%H%M%S)}
...
export SUBSET_CONFIG=${SUBSET_CONFIG:-}
export WANDB_MODE=${WANDB_MODE:-offline}

if [[ -z "$SUBSET_CONFIG" ]]; then
  PROJECT_DIR=$(cd "$SCRIPT_DIR/../../.." && pwd)
  export SUBSET_CONFIG="$PROJECT_DIR/configs/train/moca_data_ratios_smoke_cirr.yaml"
fi

echo "[mmeb_full_smoke] train run_dir=$RUN_DIR devices=$CUDA_DEVICE_LIST num_gpus=$NUM_GPUS max_steps=$MAX_STEPS subset=$SUBSET_CONFIG"
bash "$SCRIPT_DIR/run_train_full.sh"
...
if [[ -z "${CHECKPOINT:-}" || ! -d "$CHECKPOINT" ]]; then
  echo "[mmeb_full_smoke] no checkpoint found under $RUN_DIR" >&2
  exit 2
fi
if [[ ! -f "$CHECKPOINT/folder_homo.pt" ]]; then
  echo "[mmeb_full_smoke] folder_homo.pt not found under $CHECKPOINT" >&2
  exit 2
fi

export CHECKPOINT
export EVAL_MODE=${EVAL_MODE:-smoke}
export SMOKE_EVAL_MAX_QUERIES=${SMOKE_EVAL_MAX_QUERIES:-2}
export SMOKE_EVAL_MAX_LOCAL_DIDS=${SMOKE_EVAL_MAX_LOCAL_DIDS:-8}
...
export OUT_DIR=${OUT_DIR:-$RUN_DIR/eval/mmeb_smoke}
export LOG_FILE=${EVAL_LOG_FILE:-$RUN_DIR/logs/eval_mmeb_smoke_$(date +%Y%m%d_%H%M%S).log}

echo "[mmeb_full_smoke] eval checkpoint=$CHECKPOINT out_dir=$OUT_DIR"
bash "$SCRIPT_DIR/eval_mmeb_full.sh" "$CHECKPOINT"

echo "[mmeb_full_smoke] done"
echo "[mmeb_full_smoke] run_dir=$RUN_DIR"
echo "[mmeb_full_smoke] checkpoint=$CHECKPOINT"
echo "[mmeb_full_smoke] eval_dir=$OUT_DIR"
