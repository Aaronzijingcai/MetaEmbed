...
BASE_OUT_DIR=${BASE_OUT_DIR:-$RUN_DIR/eval/maxsim_compositional_iter}
...
run_eval() {
  ...
  if [[ -f "$summary" ]]; then
    echo "[maxsim_comp] skip existing $name: $summary"
    return 0
  fi
  ...
  export LOG_FILE="$BASE_LOG_DIR/eval_maxsim_comp_${name}_$(date +%Y%m%d_%H%M%S).log"
  export ONLY_EVAL_KEYWORDS="${COMPOSITIONAL_KEYWORDS[*]}"
  export AVG_METRIC
  ...
  echo "[maxsim_comp] running name=$name interaction=$interaction query_agg=$query_agg lambda=$bi_lambda topk=$topk global=$global_weight hit_penalty=$hit_penalty_weight threshold=$hit_penalty_threshold"
  echo "[maxsim_comp] keywords=${COMPOSITIONAL_KEYWORDS[*]}"
  bash "$MMEB_DIR/eval_mmeb_full.sh" "$CHECKPOINT"

  unset OUT_DIR LOG_FILE ONLY_EVAL_KEYWORDS MAXSIM_INTERACTION MAXSIM_QUERY_AGG MAXSIM_BI_LAMBDA MAXSIM_QUERY_TOPK
  unset MAXSIM_GLOBAL_WEIGHT MAXSIM_HIT_PENALTY_WEIGHT MAXSIM_HIT_PENALTY_THRESHOLD
}

# Already-known anchors, rerun here only on compositional subsets for a clean table.
...
# SMART-like global/local anchors: useful when local MaxSim overfits partial visual matches.
...
run_eval "global_bi_query_topk128_lam07_w02_sym160" "bi_query_topk" "mean" "0.7" "128" "0.2" "0.0" "0.35"

# Penalize score concentration on a few target winners without changing token budget.
...
python3 "$MMEB_DIR/compare_mmeb_runs.py" \
  ...
  --output-path "$BASE_OUT_DIR/compositional_compare.md" || true

echo "[maxsim_comp] done"
echo "[maxsim_comp] compare: $BASE_OUT_DIR/compositional_compare.md"
